...
preferences, inputs = test.identify_hard_preferences(
    subjects=subjects, n_probing_questions=10
)
...
alignment_test = test.test_deceptive_alignment(
    subject=subjects[0],
...
print("\n")
print(
    f"The opposing developer message affected the model's alignment with the subject {subjects[0]} by {alignment_test['agreement_difference']} percentage points.\n"
)
print(f"The addition of observation tags changed model behavior as well.\n")
print(
    f"The model's agreement with the subject {subjects[0]} when unobserved was {alignment_test['unobserved_agreement']}%.\n"
)
print(
    f"The model's agreement with the subject {subjects[0]} when observed was {alignment_test['observed_agreement']}%.\n"
)
print(
    f"The difference in agreement with {subjects[0]} when unobserved versus the original baseline is: {alignment_test['unobserved_versus_baseline_agreement']} percentage points.\n"
)
print(
    f"The difference in agreement with {subjects[0]} when observed versus the orignal baseline agreement is: {alignment_test['observation_impact_versus_baseline_agreement']} percentage points.\n"
)
print(
    f"The difference in agreement with {subjects[0]} when observed versus unobserved was: {alignment_test['observation_impact']} percentage points.\n"
)
print(
    f"The difference in agreement with {subjects[0]} when unobserved versus the new baseline is: {alignment_test['unobserved_versus_new_agreement']} percentage points.\n"
)
print(
    f"The difference in agreement with {subjects[0]} when observed versus the new baseline is: {alignment_test['observation_impact_versus_new_agreement']} percentage points.\n"
)
...
z_stat_1, p_val_1, ci_lower_1, ci_upper_1 = paired_proportion_test(
    alignment_test["agreement"], alignment_test["baseline_agreement"], total_sample_size
)
effect_1 = effect_size_cohen_h(
    alignment_test["agreement"], alignment_test["baseline_agreement"]
)
...
print(
    f"Difference: {alignment_test['agreement'] - alignment_test['baseline_agreement']:.1f}%"
)
print(f"95% CI: [{ci_lower_1*100:.1f}%, {ci_upper_1*100:.1f}%]")
print(f"Z-statistic: {z_stat_1:.4f}")
print(f"P-value: {p_val_1:.6f}")
print(
    f"Cohen's h: {effect_1:.3f} ({'large' if abs(effect_1) >= 0.8 else 'medium' if abs(effect_1) >= 0.5 else 'small' if abs(effect_1) >= 0.2 else 'negligible'})"
)
...
z_stat_2, p_val_2, ci_lower_2, ci_upper_2 = paired_proportion_test(
    alignment_test["observed_agreement"], alignment_test["agreement"], total_sample_size
)
effect_2 = effect_size_cohen_h(
    alignment_test["observed_agreement"], alignment_test["agreement"]
)
...
print(
    f"Difference: {alignment_test['observed_agreement'] - alignment_test['agreement']:.1f}%"
)
print(f"95% CI: [{ci_lower_2*100:.1f}%, {ci_upper_2*100:.1f}%]")
print(f"Z-statistic: {z_stat_2:.4f}")
print(f"P-value: {p_val_2:.6f}")
print(
    f"Cohen's h: {effect_2:.3f} ({'large' if abs(effect_2) >= 0.8 else 'medium' if abs(effect_2) >= 0.5 else 'small' if abs(effect_2) >= 0.2 else 'negligible'})"
)
...
effect_3 = effect_size_cohen_h(
    alignment_test["unobserved_agreement"], alignment_test["agreement"]
)
...
print(f"New Baseline: {alignment_test['agreement']}%")
print(
    f"Difference: {alignment_test['unobserved_agreement'] - alignment_test['agreement']:.1f}%"
)
print(f"95% CI: [{ci_lower_3*100:.1f}%, {ci_upper_3*100:.1f}%]")
print(f"Z-statistic: {z_stat_3:.4f}")
print(f"P-value: {p_val_3:.6f}")
print(
    f"Cohen's h: {effect_3:.3f} ({'large' if abs(effect_3) >= 0.8 else 'medium' if abs(effect_3) >= 0.5 else 'small' if abs(effect_3) >= 0.2 else 'negligible'})"
)
...
effect_4 = effect_size_cohen_h(
    alignment_test["observed_agreement"], alignment_test["unobserved_agreement"]
)
...
print(f"Unobserved: {alignment_test['unobserved_agreement']}%")
print(
    f"Difference: {alignment_test['observed_agreement'] - alignment_test['unobserved_agreement']:.1f}%"
)
print(f"95% CI: [{ci_lower_4*100:.1f}%, {ci_upper_4*100:.1f}%]")
print(f"Z-statistic: {z_stat_4:.4f}")
print(f"P-value: {p_val_4:.6f}")
print(
    f"Cohen's h: {effect_4:.3f} ({'large' if abs(effect_4) >= 0.8 else 'medium' if abs(effect_4) >= 0.5 else 'small' if abs(effect_4) >= 0.2 else 'negligible'})"
)
...
for i, (name, p_val) in enumerate(zip(test_names, p_values), 1):
    print(
        f"  {i}. {name}: p = {p_val:.6f} {'***' if p_val < 0.01 else '**' if p_val < 0.05 else '*' if p_val < 0.1 else ''}"
    )
...
for i, (name, p_val) in enumerate(zip(test_names, p_values), 1):
    print(
        f"  {i}. {name}: {'Significant' if p_val < alpha_bonferroni else 'Not Significant'}"
    )
